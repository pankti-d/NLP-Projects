...
# A route to return result after checking the utterance.
@app.route('/checkUtterance/', methods=['POST'])
def check_utterance():

    ##Code to get botid from the request query [will be used when we test query]
    ...
    #
    # result = list(collection.find({'isDefault': False, 'botId': ObjectId(botid)},
    #                               {'botId': True, 'intentId': True, 'trainingPhrases': True, 'isArchived':False}))[1:6]
